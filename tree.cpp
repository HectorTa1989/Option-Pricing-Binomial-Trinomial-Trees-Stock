...
double ** FeedForward(double ** Z, double S, int s, int m, int n, double U, double D)
{
    ...
    while(cs <= n){
        ...
        for(int i = cs + 1; i < n; ++i){
            ...
            Z[s - (i - cs)*upx][i] = Z[s - (i - cs - 1)*upx][i - 1]*U;
            Z[s + (i - cs)*dnx][i] = Z[s + (i - cs -1)*dnx][i - 1]*D;
        }
        // Increment tree
        cs += 2;
    }
...
double ** Payoff(double ** Z, double K, int m, int n, std::string opType){
    ...
    while(b <= m){
        ...
        if(opType == "call"){
            Z[b + 1][n - 1] = std::max(Z[b][n - 1] - K, 0.0);
        } else {
            Z[b + 1][n - 1] = std::max(K - Z[b][n - 1], 0.0);
        }
        b += 4;
    }
...
double ** Discount(double ** Z, double P, double nP, double r, double dt, int m, int n, std::string opType){
    ...
    for(int i = 1; i < n; ++i){
        ...
        while(ls < m - osx){
            ...
            if(opType == "call"){
                Z[ls - 1][cx] = std::max(exp(-r*dt)*(P*Z[ls - 3][cx + 1] + nP*Z[ls + 1][cx + 1]) , 0.0);
            } else {
                Z[ls - 1][cx] = std::max(exp(-r*dt)*(nP*Z[ls - 3][cx + 1] + P*Z[ls + 1][cx + 1]) , 0.0);
            }
            ls += 4;
        }
        osx += 2;
